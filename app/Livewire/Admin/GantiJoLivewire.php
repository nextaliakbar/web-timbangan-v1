...
class GantiJoLivewire extends Component
{
    ...
    private function update()
    {
        $count = StfStatus::where('NO_JO', '=', $this->noJoBaru)
            ...
            if($count > 1) {
                $this->dispatch('closeModalEdit', title: 'Perbarui No JO', text: 'No JO sudah di STFkan', icon: 'error');
                return;
            }
            ...
            $this->dispatch('closeModalEdit', title:'Perbarui No JO', text: 'No JO berhasil diperbarui', icon: 'success');
    }
    ...
    public function mainUpdate()
    {
        if(filled($this->noJoBaru)) {
            ...
            $this->dispatch('modalConfirmUpdate');
        }
    }
